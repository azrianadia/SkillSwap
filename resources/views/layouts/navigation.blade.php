        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent hover:opacity-80 transition-opacity">
                        KolaboKampus
                    </a>
                </div>
            </div>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center space-x-2 px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div class="h-8 w-8 rounded-full flex items-center justify-center overflow-hidden bg-gray-100 relative">
                                @if (Auth::user()->avatar)
                                ...
                                @endif
                                @if (Auth::user()->is_pro)
                                    <span class="absolute -bottom-1 -right-1 w-5 h-5 bg-yellow-400 rounded-full flex items-center justify-center">
                            ...
                            <div>{{ Auth::user()->name }} @if(Auth::user()->is_pro)<span class="ml-1 text-yellow-400">★</span>@endif</div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4 flex items-center space-x-3">
                <div class="h-10 w-10 rounded-full flex items-center justify-center overflow-hidden bg-gray-100 relative">
                    @if (Auth::user()->avatar)
                        <img src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="{{ Auth::user()->name }}" class="h-full w-full object-cover" />
                    @else
                ...
                <div class="flex-1 min-w-0">
                    <div class="font-medium text-base text-gray-800 truncate">{{ Auth::user()->name }} @if(Auth::user()->is_pro)<span class="ml-1 text-yellow-400">★</span>@endif</div>
